 ...
 *
 * 사용:
 *   node scripts/apply-sohssure-analysis.mjs              # dry-run (변경 없음)
 *   node scripts/apply-sohssure-analysis.mjs --apply-db   # 실제 적용
 */
...
async function main() {
  ...
  const analysis = JSON.parse(fs.readFileSync(path.join(GEN, 'sohssure_analysis_filled.json'), 'utf8'));
  ...
  try {
    ...
    const [paperRows] = await conn.execute(
      "SELECT id, payload_json FROM test_papers WHERE id = 'diag_paper_sohssure'",
    );
    if (paperRows.length === 0) throw new Error('test_papers diag_paper_sohssure 없음');
    ...
    const [diagQRows] = await conn.execute(
      "SELECT id, passage_id, order_in_passage, correct_choice, choices_json FROM diag_questions WHERE tier = 'sohssure' ORDER BY passage_id, order_in_passage, id",
    );
    ...
    const backupPath = path.join(GEN, `sohssure_apply_backup_${runStamp}.json`);
    ...
    const filledPath = path.join(GEN, `sohssure_payload_filled_${runStamp}.json`);
    ...
    try {
      const [r1] = await conn.execute(
        "UPDATE test_papers SET payload_json = ?, updated_at = NOW() WHERE id = 'diag_paper_sohssure'",
        [newPayloadJson],
      );
      ...
      const testId = 'diag_paper_sohssure';
