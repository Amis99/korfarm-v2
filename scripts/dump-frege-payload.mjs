#!/usr/bin/env node
/**
 * 프레게 진단 테스트(diag_paper_frege) payload_json 추출.
 *
 * 출력:
 *  - generated/frege_payload_input.json      (작업용 원본)
 *  - generated/frege_payload_backup_{stamp}.json (롤백용 백업)
 *
 * 사용: node scripts/dump-frege-payload.mjs
 */
...
async function main() {
  ...
  try {
    ...
    const [paperRows] = await conn.execute(
      "SELECT id, title, payload_json FROM test_papers WHERE id = 'diag_paper_frege'",
    );
    if (paperRows.length === 0) {
      throw new Error('test_papers.id=diag_paper_frege 행이 없습니다.');
    }
    ...
    const [diagQRows] = await conn.execute(
      "SELECT id, passage_id, order_in_passage, stem, correct_choice, choices_json FROM diag_questions WHERE tier = 'frege' ORDER BY passage_id, order_in_passage, id",
    );
    console.log(`diag_questions (tier=frege): ${diagQRows.length}개`);

    const [diagPRows] = await conn.execute(
      "SELECT id, level, genre, text_md FROM diag_passages WHERE tier = 'frege' ORDER BY id",
    );
    console.log(`diag_passages (tier=frege): ${diagPRows.length}개`);
    ...
    const inputPath = path.join(OUT_DIR, 'frege_payload_input.json');
    fs.writeFileSync(inputPath, JSON.stringify(dump, null, 2), 'utf8');
    console.log(`작업용 저장 → ${inputPath}`);

    const backupPath = path.join(OUT_DIR, `frege_payload_backup_${runStamp}.json`);
