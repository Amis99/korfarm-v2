 ...
 *
 * 사용:
 *   node scripts/apply-wittgenstein-analysis.mjs              # dry-run (변경 없음)
 *   node scripts/apply-wittgenstein-analysis.mjs --apply-db   # 실제 적용
 */
...
async function main() {
  ...
  const analysis = JSON.parse(fs.readFileSync(path.join(GEN, 'wittgenstein_analysis_filled.json'), 'utf8'));
  ...
  try {
    ...
    const [paperRows] = await conn.execute(
      "SELECT id, payload_json FROM test_papers WHERE id = 'diag_paper_wittgenstein'",
    );
    if (paperRows.length === 0) throw new Error('test_papers diag_paper_wittgenstein 없음');
    ...
    const [diagQRows] = await conn.execute(
      "SELECT id, passage_id, order_in_passage, correct_choice, choices_json FROM diag_questions WHERE tier = 'wittgenstein' ORDER BY passage_id, order_in_passage, id",
    );
    ...
    const backupPath = path.join(GEN, `wittgenstein_apply_backup_${runStamp}.json`);
    ...
    const filledPath = path.join(GEN, `wittgenstein_payload_filled_${runStamp}.json`);
    ...
    try {
      const [r1] = await conn.execute(
        "UPDATE test_papers SET payload_json = ?, updated_at = NOW() WHERE id = 'diag_paper_wittgenstein'",
        [newPayloadJson],
      );
      ...
      const testId = 'diag_paper_wittgenstein';
