...
- **WITTGENSTEIN 레벨**: `비문학 → 문학 → 비문학 → 화법 → 작문 → 비문학 → 문법` 7일 순환을 사용한다.
  ...
  - `(가)/(나)` 결합형 source는 **한 학습 파일에 반드시 하나만 사용**한다. 제목, 지문, 후속 학습은 선택한 한 부분만 기준으로 만들고, 다른 부분의 내용이나 표현을 섞지 않는다.
  - 화법 day는 실제 **토론·토의·협상·대담·발표 대본/발화문**으로, 작문 day는 **교정 포인트가 있는 초고**로, 문법 day는 **문법 설명문**으로 수동 작성한다.
  - 문법 day는 `문장 호응`, `중의성`, `어문 규범`, `문법 개념 적용` 같은 넓은 우산 제목을 반복하지 말고, 가능한 한 **세부 문법 주제**를 직접 제목과 지문에 반영한다.
  - 문법 주제는 `구개음화`, `음소 배열 제약`, `이형태`, `사잇소리 현상`, `보조 용언`, `피동 접미사와 사동 접미사`, `중세국어의 객체 높임 표현`, `중세국어의 주격 조사`, `훈민정음 창제 원리`처럼 작게 쪼갠다.
  - 한 레벨 안에서는 같은 문법 축을 짧은 간격으로 반복하지 않는다. 특히 `문장 호응`, `중의성`처럼 포괄적인 제목은 세부 항목으로 바꿔 분산 배치한다.
...
- `(가)/(나)` 결합형 source는 한 파일에 한 부분만 쓴다. 제목과 지문, 나중에 붙는 학습 요소도 모두 선택한 그 한 부분만 근거로 작성한다.
- 화법 day는 실제 **토론·토의·협상·대담·발표 대본/발화문**으로, 작문 day는 **교정 포인트가 있는 초고**로, 문법 day는 **문법 설명문**으로 수동 입력한다.
- 비트겐슈타인 문법 day는 broad label을 반복하지 않고 `구개음화`, `음소 배열 제약`, `이형태`, `사잇소리 현상`, `중세국어의 객체 높임 표현`처럼 **세부 주제 중심**으로 배치한다.
- 같은 레벨 안에서 `문장 호응`, `중의성`, `어문 규범` 같은 우산 제목을 잦게 반복하지 않는다. 배치표와 실제 title은 가능한 한 구체적인 문법 항목명으로 맞춘다.
