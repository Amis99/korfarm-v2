...
- **WITTGENSTEIN 레벨**: `비문학 → 문학 → 비문학 → 화법 → 작문 → 비문학 → 문법` 7일 순환을 사용한다.
  ...
  - 화법·작문·문법 day는 고교형 평가 문항 축에 맞게 수동 작성한다.
...
- 지문 입력 단계에서는 문학은 원문 그대로 발췌해 넣고, 비문학/화법/작문/문법은 배치표를 참고하되 실제 파일에서는 **구체 개념명**을 제목과 지문에 반영해 수동 입력한다.
...
- 구조 통일 단계에서는 이 배치표 기준으로 `title`, `subArea`, 빈 `payload` 구조를 먼저 맞춘다.
- 지문 입력 단계에서는 문학은 작품 원문 그대로 발췌하고, 비문학/화법/작문/문법은 배치표에 적힌 주제대로 지문만 수동 입력한다.
...
- 화법·작문·문법 day는 고교형 평가 축에 맞게 수동 입력한다.
