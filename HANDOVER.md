...
- **WITTGENSTEIN 레벨**: `비문학 → 문학 → 비문학 → 화법 → 작문 → 비문학 → 문법` 7일 순환을 사용한다.
  ...
  - `참고용 기출 지문/` source에 `(가)`와 `(나)`가 함께 있으면 한 파일에 둘 다 쓰지 말고, 필요한 하나만 떼어 사용한다. 문학은 선택한 한 부분만 **그대로 발췌**하고, 비문학은 선택한 한 부분만 바탕으로 **변형 수록**한다.
...
- `참고용 기출 지문/` source가 `(가)/(나)` 결합형이면 두 지문을 합치지 말고, 필요한 한 부분만 떼어 사용한다. 문학은 그 한 부분을 그대로 발췌하고, 비문학은 그 한 부분만 기준으로 변형 수록한다.
