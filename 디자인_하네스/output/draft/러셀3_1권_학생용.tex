...
\vspace*{40mm}
\begin{center}
{\Huge\bfseries\color{kfPrimary} 러셀3}\\[10pt]
...
\kfStartGrammar{문법 영역 — 단어의 이름표, 품사 체계의 이해}


\kfSecHeader{kfAreaGrammar}{개념}{단어의 이름표, 품사 체계의 이해}
...
\noindent\textit{\small 지문의 핵심 내용을 떠올리며 아래 빈칸에 알맞은 말을 채워 보세요. \#\#\# 1. 품사 분류 기준 | 기준 | 분류 | | :---- | :---- | | 1. 형태 (형태 변화 여부) | ∙ [ ① ]: 형태 고정 (예: 하늘, 매우)<br>∙ [ ② ]: 형태 변화 (예: 신다 → 신고, 신으니) | | 2. 기능 (문장 속 역할) | ∙ [ ③ ]: 문장의 뼈대 (주어, 목적어 등)<br>∙ [ ④ ]: 문법적 관계 표시<br>∙ [ ⑤ ]: 문장의 서술어 기능 (활용 O)<br>∙ [ ⑥ ]: 다른 말을 꾸며 줌 (활용 X)<br>∙ [ ⑦ ]: 독립적으로 쓰임 | | 3. 의미 (단어의 공통된 뜻) | ∙ 9품사 (명사, 대명사, 수사, 조사, 동사, 형용사, 관형사, 부사, 감탄사) | \#\#\# 2. 9품사 분류 (기능과 의미 중심) | 기능 (5) | 품사 (9) | 의미 및 특징 | | :---- | :---- | :---- | | [ ③ ] | 명사 | 대상의 이름을 나타냄 (예: 학교, 우정) | | | [ ⑧ ] | 명사를 대신하여 가리킴 (예: 나, 우리, 이것) | | | [ ⑨ ] | 수량이나 순서를 나타냄 (예: 하나, 첫째) | | [ ④ ] | [ ⑩ ] | 주로 체언 뒤에 붙어 관계를 표시함 (예: 가, 를, 에서)<br>\textit{(단, [ ⑪ ] '이다'는 유일하게 가변어에 속함)} | | [ ⑤ ] | [ ⑫ ] | 대상의 움직임을 나타냄 (예: 가다, 살다) | | | [ ⑬ ] | 대상의 성질/상태를 나타냄 (예: 멋지다, 부드럽다) | | [ ⑥ ] | [ ⑭ ] | 오직 체언 (명사, 대명사, 수사)만 꾸밈 (예: 새, 모든) | | | [ ⑮ ] | 주로 용언 (동사, 형용사)을 꾸밈 (예: 방긋, 참) | | [ ⑦ ] | [ ⑯ ] | 놀람, 대답, 부름을 나타냄 (예: 어머나, 네) | \#\#\# 3. 심화 확인 (형태 기준) * 가변어에 속하는 3가지 품사: [ ⑰ ], [ ⑱ ], 그리고 [ ⑲ ] '이다' * 동사와 형용사의 구분 방법: 현재형 어미 [ ⑳ ], 명령형 '-아라', 청유형 '-자'와 결합할 수 있으면 동사, 없으면 형용사이다. ---}\par\medskip
...
\begin{kfEvidence}길이 (가)\uline{밝다}. 날이 (나)\uline{밝는다}.\end{kfEvidence}
\item 다음 문장의 밑줄 친 단어들의 품사를 각각 쓰시오.
\begin{kfEvidence}(가) \uline{저} 하늘을 보아라. (나) \uline{저기}에 우리 집이 있다.\end{kfEvidence}
\item 다음 문장의 밑줄 친 단어들의 품사를 각각 쓰시오.
\begin{kfEvidence}(가) 학생 \uline{다섯}이 교실에 있다 (나) \uline{다섯} 학생이 교실에 있다.\end{kfEvidence}
...
\begin{kfQBlock}{1}{}
\kfQStem{품사 분류 기준에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{‘용언’에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{동사와 형용사를 구분하는 방법에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      ( ㉠ )은/는 체언을 꾸며주며, ( ㉡ )은/는 주로 용언을 꾸며준다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠ 관형사 / ㉡ 부사
\kfChoice ㉠ 부사 / ㉡ 관형사
\kfChoice ㉠ 명사 / ㉡ 동사
...
\end{kfQBlock}
\begin{kfQBlock}{11}{}
\kfQStem{다음 문장에 사용되지 \uline{않은} 품사는?}
...
\begin{kfEvidence}<보기>       하늘, 나, 첫째, 가, 먹다, 예쁘다, 새, 매우, 아\end{kfEvidence}
\begin{kfChoices}
...
\kfStartConcept{개념 영역 — 구수한 사투리의 맛, 방언의 효과}


\kfSecHeader{kfAreaConcept}{개념}{구수한 사투리의 맛, 방언의 효과}
...
\begin{multicols}{2}\raggedcolumns\setlength{\columnsep}{12pt}
\begin{kfQBlock}{1}{}
\kfQStem{'방언'에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
\kfChoice 사투리라고도 부른다.
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{‘향토성’에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{방언의 효과로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
\kfChoice 향토성을 더해준다.
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{위 글을 바탕으로 할 때, 방언을 사용한 소설을 읽는 독자의 반응으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}< 보기 >      "느 집엔 이거 없지?"      하고 섰다. 그 다음에는 “느 아버지가 고자라지?” 하며 놀렸다.\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}< 보기 >      "허 생원은 걸핏하면 이 길을 기억하니께."      "그렇지, 밤길을 걸으면서도 이 길을 걸은 지가 이십 년이나 됐네."\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}< 보기 >      소설을 읽으니 마치 내가 강원도 봉평 장터에 직접 가 있는 듯한 생생한 느낌이 들었다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 향토성
\kfChoice 현장감
\kfChoice 친근감
...
\begin{kfQBlock}{8}{}
\kfQStem{소설에서 방언을 사용하여 얻는 효과 중 <보기>가 설명하는 것은 무엇인가?}
\begin{kfEvidence}< 보기 >      주인공이 사용하는 구수한 사투리를 들으니 왠지 모르게 정겹고 그 마을 특유의 분위기가 느껴졌다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 향토성
\kfChoice 비판성
\kfChoice 주관성
...
\kfSecHeader{kfAreaLiterature}{지문}{문장 독해 — 발췌}
...
\kfSecHeader{kfAreaLiterature}{지문}{해설 학습 1}
...
\kfSecHeader{kfAreaLiterature}{지문}{해설 학습 2}
...
\kfSecHeader{kfAreaLiterature}{지문}{해설 학습 3}
...
\end{kfQBlock}
\begin{kfQBlock}{1}{}
\kfQStem{이 글에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{영식의 성격 변화로 적절하지 \uline{않은} 것은 무엇인가?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{남편이 산신에게 제사를 지내려는 태도에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{㉠\textasciitilde{}㉢에 대한 설명으로 적절하지 \uline{않은} 것은 무엇인가?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{7}{}
\kfQStem{이 글의 결말에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{8}{}
\kfQStem{<보기>의 관점에서 윗글을 감상한 내용으로 적절하지 \uline{않은} 것은 무엇인가?}
\begin{kfEvidence}<보기>   김유정의 소설에는 당대 농촌의 어려운 현실이 잘 드러나 있다. 작가는 가난 때문에 비극적인 상황에 놓인 인물들의 어리석은 행동을 웃음으로 그려내지만, 그 이면에는 현실에 대한 날카로운 비판과 인간에 대한 깊은 연민을 담고 있다.\end{kfEvidence}
\begin{kfChoices}
...
\kfStartNonlit{비문학 영역 — [인문] 용서와 추서, 정약용의 실천 윤리}


\kfSecHeader{kfAreaNonlit}{지문}{[인문] 용서와 추서, 정약용의 실천 윤리}
...
\noindent\textit{\small 다음 뜻풀이에 해당하는 어휘를 지문에서 찾아 쓰시오.}\par\medskip
\begin{kfVocab}
...
\kfSecHeader{kfAreaNonlit}{지문}{문장 독해 — 본문}
...
\noindent\textit{\small 다음 글의 구조를 정리한 표의 빈칸을 채워 보세요.}\par\medskip
...
글의 전체적인 논리 흐름을 파악하며 빈칸에 알맞은 내용을 채워 보세요. \\

1문단: 정약용 사상의 배경 \\

\noindent \textbullet\ 연구 동기: 인간의 ( ① ) 행위를 설명하고자 함   \\
\noindent \textbullet\ 연구 방법: ( ② )을 비판하며 자신만의 이론 정립 \\

2문단: 주희(주자학)의 인간 본성론 \\

\noindent \textbullet\ 본연지성: 하늘로부터 받은 순수하고 ( ③ ) 본성   \\
\noindent \textbullet\ 기질지성: 본연지성에 각기 다른 ( ④ )이 더해진 것 \\

3문단: 정약용의 주자학 비판 \\

\noindent \textbullet\ 비판: 기질에 따라 선악이 결정된다면, 인간의 ( ⑤ )가 개입되지 않음   \\
\noindent \textbullet\ 결론: 악한 행위에 ( ⑥ )을 물을 수 없음 \\

4문단: 정약용의 본성론 - 기호 \\

\noindent \textbullet\ 본성의 정의: 즐기고 좋아하는 성향, 즉 ‘( ⑦ )’ \\

5문단: 인간 기호의 종류 \\

\noindent \textbullet\ ( ⑧ )에서 나온 기호 (몸의 성향)   \\
\noindent \textbullet\ ( ⑨ )에서 나온 기호 (영혼의 성향) \\

6문단: 선악의 결정 \\

\noindent \textbullet\ 과정: 감각적 욕구 제어 실패 시 악, ( ⑩ ) 욕구 선택 시 선   \\
\noindent \textbullet\ 핵심: 선악의 선택은 온전히 인간의 ( ⑪ )에 달려 있음 \\

7문단: 선한 행위의 실천 방법 - 서(恕) \\

\noindent \textbullet\ 방법 제시: ( ⑫ )를 강조   \\
\noindent \textbullet\ 구분: 용서(容恕)와 ( ⑬ )(推恕)로 구분 \\

8문단: 용서와 추서의 의미 \\

\noindent \textbullet\ ( ⑭ ): 타인의 악을 너그럽게 보아줌   \\
\noindent \textbullet\ ( ⑮ ): 내가 대접받고 싶은 대로 타인을 대우함   \\
\noindent \textbullet\ 추서의 실천: ( ⑯ )의 마음을 미루어 ( ⑰ )의 처지를 생각함 \\

9문단: 추서 강조의 이유 \\

\noindent \textbullet\ ( ⑱ )의 문제점: 타인의 악한 행위를 그냥 넘어갈 수 있음   \\
\noindent \textbullet\ ( ⑲ )의 장점: 타인을 이해하고 선한 행위를 실천하게 함 \\
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{주희와 정약용의 견해를 비교한 것으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{㉠추서(推恕)에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      한 제자가 스승에게 물었다. "사람들은 왜 착하게 살지 못합니까?"      스승이 답했다. "타고난 기질 때문이라고 핑계를 대서는 안 된다. 맛있는 음식을 먹고 싶은 마음과 어려운 사람을 돕고 싶은 마음 사이에서 무엇을 따를지 결정하는 것은 너 자신에게 달려있기 때문이다."\end{kfEvidence}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{윗글의 중심 사상에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\kfStartGrammar{문법 영역 — 단어의 구조를 분석하는 틀: 형태소, 어근, 접사}


\kfSecHeader{kfAreaGrammar}{개념}{단어의 구조를 분석하는 틀: 형태소, 어근, 접사}
...
\kfSecHeader{kfAreaGrammar}{활동}{내용 확인}
\noindent\textit{\small 지문의 핵심 내용을 떠올리며 아래 빈칸에 알맞은 말을 채워 보세요. 1. 형태소의 개념과 분류 | 구분 | 내용 | | :---- | :---- | | 형태소 | ∙ [ ① ]을/를 가진 채로 더는 쪼갤 수 없는 가장 작은 말의 단위. | | 분류 기준 1: [ ② ] | ∙ 자립 형태소: 문장에서 [ ③ ] 쓸 수 있음. (예: 하늘, 책)<br>∙ 의존 형태소: 반드시 다른 말에 [ ④ ] 쓰임. (예: -다, -이/가, 가-) | | 분류 기준 2: [ ⑤ ] | ∙ [ ⑥ ]: 구체적인 대상, 동작, 상태 등 실질적 의미를 가짐. (예: 하늘, 가-)<br>∙ [ ⑦ ]: 문법적인 관계를 표시하거나 보조적인 의미만 더함. | | [ ⑦ ]의 종류 | ∙ 문법적 관계 표시: [ ⑧ ] (예: -이/가)<br>∙ 활용 시 변하는 부분: [ ⑨ ] (예: -다, -고)<br>∙ 어근에 붙어 뜻을 더함: [ ⑩ ] (예: 맨-, -개) | 2. 형태소 2x2 분류 | (기준) | 실질 형태소 (뜻 O) | 형식 형태소 (뜻 X) | | :---- | :---- | :---- | | 자립 형태소 (홀로 O) | ∙ 명사, 대명사, 수사, 관형사, 부사, 감탄사<br>∙ (예: 하늘, 책, 매우) | ∙ (없음) | | 의존 형태소 (홀로 X) | ∙ [ ⑪ ]<br>∙ (예: '먹다'의 '먹-') | ∙ [ ⑫ ], [ ⑬ ], [ ⑭ ]<br>∙ (예: -이/가, -다, -개) |  핵심 정리: 국어에서 유일하게 '의존 형태소이면서 동시에 실질 형태소' 인 것은 바로 [ ⑮ ] 이다. 3. 단어의 구조 분석 | 분석 관점 | 핵심 개념 | 설명 | 예시 | | :---- | :---- | :---- | :---- | | 단어 형성 | [ ⑯ ] | ∙ 단어에서 실질적인 의미를 나타내는 핵심 부분 | ∙ '덮개'의 '덮-'<br>∙ '맨손'의 '손' | | | [ ⑰ ] | ∙ 어근에 붙어 뜻을 더하거나 제한하는 부분<br>-[ ⑱ ] : 어근 앞에 붙음<br>-[ ⑲ ] : 어근 뒤에 붙음 | ∙ '덮개'의 '-개'<br>∙ '맨손'의 '맨-' | | 용언 활용 | [ ⑳ ] | ∙ 용언이 활용할 때 변하지 않는 부분 | ∙ '먹다, 먹고'의 '먹-' | | | [ ㉑ ] | ∙ 용언이 활용할 때 변하는 부분 | ∙ '먹다, 먹고'의 '-다, -고' | 4. 어근 vs 어간 심화 비교 | 단어 | 분석 | | :---- | :---- | | 먹다 | ∙ 어근 (실질적 핵심): [ ㉒ ]<br>∙ 어간 (활용 시 불변): [ ㉓ ] | | 치솟다 | ∙ 어근 (실질적 핵심): [ ㉔ ]<br>∙ 어간 (활용 시 불변: '치솟고, 치솟으니'): [ ㉕ ] | ---}\par\medskip
...
\item '먹었다'의 어간 '먹-'은 (자립/의존) 형태소이면서 (실질/형식) 형태소입니다. ( ) 안에 알맞은 말에 O 표시 하시오.
...
\begin{multicols}{2}\raggedcolumns\setlength{\columnsep}{12pt}
\begin{kfQBlock}{1}{}
\kfQStem{형태소에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{<보기>의 ㉠\textasciitilde{}㉣에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>      ㉠ 하늘        ㉡ 이       ㉢ 푸르        ㉣ 다\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      ∙ 홀로 쓰일 수 없는 의존 형태소이다.      ∙ 실질적인 의미를 가진 실질 형태소이다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 높-
\kfChoice 하늘
\kfChoice -이
...
\end{kfQBlock}
\begin{kfQBlock}{7}{}
\kfQStem{‘어근’과 ‘접사’에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{11}{}
\kfQStem{‘어근’과 ‘어간’에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{12}{}
\kfQStem{다음 중 밑줄 친 부분이 '접사'가 \uline{아닌} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>       ㉠ 의존 형태소이다.       ㉡ 실질 형태소이다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 하늘
\kfChoice 를
\kfChoice -다
...
\end{kfQBlock}
\begin{kfQBlock}{1}{}
\kfQStem{뜻을 가진 채로 더는 쪼갤 수 없는 가장 작은 말의 단위를 무엇이라고 하는가?}
\kfAnswerLines{1}
\end{kfQBlock}
\begin{kfQBlock}{2}{}
...
\kfStartConcept{개념 영역 — 판소리에서 태어난 소설}


\kfSecHeader{kfAreaConcept}{개념}{판소리에서 태어난 소설}
...
\begin{multicols}{2}\raggedcolumns\setlength{\columnsep}{12pt}
\begin{kfQBlock}{1}{}
\kfQStem{판소리계 소설의 형성 배경에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{판소리계 소설에 운문체와 산문체가 섞여 있는 이유에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{판소리계 소설의 내용적 특징으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}< 보기 >      (가) 「춘향전」에서 백성을 괴롭히는 나쁜 관리 '변학도'를 비판함      (나) 「흥보전」에서 욕심 많고 심술궂은 양반 '놀보'를 비판함\end{kfEvidence}
\begin{kfChoices}
\kfChoice 풍자
\kfChoice 해학
\kfChoice 우연성
...
\begin{kfEvidence}< 보기 >      "돈 나온다, 돈! 돈! 돈! 엽전 한 꿰미, 은돈 한 꿰미, 황금 한 꿰미... (중략) ... 쌀 나온다, 쌀! 쌀! 쌀! 흰 쌀, 검은 쌀, 찹쌀, 멥쌀..."\end{kfEvidence}
\begin{kfChoices}
\kfChoice 서술자의 개입
\kfChoice 열거법과 반복법
\kfChoice 우연적인 사건 전개
...
\begin{kfEvidence}< 보기 >      "일(一)자로 나를 치니 일편단심 굳은 마음 변할쏘냐. 이(二)자로 나를 치니 이부불경 굳은 절개 변할쏘냐. 삼(三)자로 나를 치니 삼청동 우리 도련님..."\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}< 보기 >      "아이고, 이게 누군가! 눈먼 심 봉사 아니시오? 내가 봉사님 눈 뜨게 해 줄 약을 가져왔으니... (중략) 어이쿠, 내가 속였네! 돈이나 내놓으시오!"      (이 부분은 '뺑덕어멈'이라는 인물이 심 봉사를 속이는 장면이다.)\end{kfEvidence}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{이 글의 중심 소재인 ‘쌀 삼백 석’의 기능으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{㉠\textasciitilde{}㉤에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{이 글의 결말이 주는 교훈으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{이 소설의 핵심 주제에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{8}{}
\kfQStem{<보기>의 관점에서 이 글을 이해한 내용으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>

고전 소설에는 현실에서 일어나기 어려운 비현실적이고 신비로운 사건들이 자주 등장한다. 이러한 장치들은 주인공이 겪는 시련의 고통을 극복하게 하고, ‘선한 인물은 복을 받는다’는 주제 의식을 효과적으로 드러내는 역할을 한다.\end{kfEvidence}
\begin{kfChoices}
...
\kfStartNonlit{비문학 영역 — [사회] 국가가 처벌하는 형사법 vs 개인이 다투는 민사법}


\kfSecHeader{kfAreaNonlit}{지문}{[사회] 국가가 처벌하는 형사법 vs 개인이 다투는 민사법}
...
\end{kfQBlock}
\begin{kfQBlock}{18}{}
\kfQStem{형량이 달라질 수 있는 요인으로 언급되지 \uline{않은} 것은?}
\begin{kfChoices}
\kfChoice 동기
\kfChoice 상황
\kfChoice 날씨
...
\end{kfQBlock}
\begin{kfQBlock}{35}{}
\kfQStem{다음 중 형사법과 민사법의 차이점으로 언급되지 \uline{않은} 것은?}
\begin{kfChoices}
\kfChoice 목적
\kfChoice 절차
\kfChoice 입증 정도
...
\noindent \textbullet\ 형사법   \\
\noindent \quad\textbullet\ 관계: ( ① )와 범죄자 사이   \\
\noindent \quad\textbullet\ 목적: 사회 질서 유지, 범죄 ( ② )   \\
\noindent \textbullet\ 민사법   \\
\noindent \quad\textbullet\ 관계: ( ③ ) 간의 관계   \\
\noindent \quad\textbullet\ 목적: 개인 간 분쟁 해결, 개인의 ( ④ ) 보호 \\

3문단: 형사 소송의 절차와 특징 \\

\noindent \textbullet\ 당사자: ( ⑤ )(공익 대표) vs 피고인(범죄 혐의자)   \\
\noindent \textbullet\ 입증 책임: 검사가 피고인의 ( ⑥ )를 입증해야 함   \\
\noindent \textbullet\ 판결: 법원이 범죄 성립 여부와 잘못의 정도를 따져 형벌 결정 \\

4문단: 민사 소송의 절차와 특징 \\

\noindent \textbullet\ 당사자: ( ⑦ )(소송 제기) vs 피고(가해자로 지목)   \\
\noindent \textbullet\ 입증 책임: 양쪽 모두 자신에게 ( ⑧ )이 없음을 입증해야 함   \\
\noindent \textbullet\ 특징: 당사자 간 ( ⑨ )이 이루어지면 소송 종료 가능 \\

5-6문단: 두 법의 관계 및 가장 큰 차이점 \\

\noindent \textbullet\ 관계: 하나의 사건에 두 소송 모두 가능하지만, 결론은 다를 수 있음   \\
\noindent \textbullet\ 가장 큰 차이점: ( ⑩ )의 정도   \\
\noindent \quad\textbullet\ 형사 소송: ‘합리적인 의심을 할 여지가 없을 정도’의 강한 입증 요구 ( ⑪ ) 원칙 때문   \\
\noindent \quad\textbullet\ 민사 소송: ‘일반인이라면 의심하지 않을 정도’의 입증 요구 \\

7문단: 결론 \\

\noindent \textbullet\ 형사법과 민사법은 목적, 절차, 입증 정도가 다른 ( ⑫ )의 법 분야임 \\
\end{tcolorbox}\par\medskip
\kfSecHeader{kfAreaNonlit}{문제}{}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{형사 소송과 민사 소송을 비교한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>       친구에게 빌려준 돈을 받지 못한 A는 법원에 소송을 제기했다. 이 소송에서 A는 ( ㉠ )이/가 되고, 친구는 ( ㉡ )이/가 된다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 원고 - 피고
\kfChoice 피고 - 원고
\kfChoice 검사 - 피고인
...
\begin{kfEvidence}<보기>      폭행 혐의로 기소된 연예인 B는 형사 재판에서 증거 불충분으로 무죄 판결을 받았다. 하지만 피해자가 제기한 민사 소송에서는 패소하여 치료비와 위자료를 지급하라는 판결을 받았다.\end{kfEvidence}
\begin{kfChoices}
...
\kfStartGrammar{문법 영역 — 새로운 단어의 탄생: 파생어와 합성어}


\kfSecHeader{kfAreaGrammar}{개념}{새로운 단어의 탄생: 파생어와 합성어}
...
\kfSecHeader{kfAreaGrammar}{활동}{내용 확인}
\noindent\textit{\small 지문의 핵심 내용을 떠올리며 아래 빈칸에 알맞은 말을 채워 보세요. 1. 단어의 분류 | 분류 | 개념 | 예시 | | :---- | :---- | :---- | | [ ① ] | ∙ 하나의 어근으로만 이루어진 단어 | ∙ 하늘, 바다, 먹다 | | [ ② ] | ∙ 둘 이상의 형태소가 결합한 단어 | ∙ 맨손, 돌다리, 덮개 | 2. 복합어의 종류 (단어 형성법) | 형성법 | 개념 (짜임) | 세부 분류 | 예시 | | :---- | :---- | :---- | :---- | | [ ③ ] | ∙ 어근 + [ ④ ] | 접두 파생어<br>([ ⑤ ]가 어근 앞에 붙음) | ∙ [ ⑥ ] + 손 → 맨손<br>∙ [ ⑦ ] + 사과 → 풋사과<br>∙ 군- + 소리 → 군소리 | | | | 접미 파생어<br>([ ⑧ ]가 어근 뒤에 붙음) | ∙ 장난 + [ ⑨ ] → 장난꾸러기<br>∙ 덮- + [ ⑩ ] → 덮개 (도구)<br>∙ 낚시 + -질 → 낚시질 (행위) | | [ ⑪ ] | ∙ 어근 + [ ⑫ ] | 명사+명사 | ∙ 돌 + 다리 → [ ⑬ ]<br>∙ 책 + 가방 → 책가방 | | | | 용언 어근 결합 | ∙ 오- + 가- → [ ⑭ ]<br>∙ 검- + 붉- → [ ⑮ ] | | | | 기타 결합 | ∙ 새 + 해 → [ ⑯ ]<br>∙ 큰 + 집 → 큰집 |}\par\medskip
...
\begin{multicols}{2}\raggedcolumns\setlength{\columnsep}{12pt}
\begin{kfQBlock}{1}{}
\kfQStem{단어의 짜임에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      하늘, 손발, 덧신, 구름\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      ㉠ 단일어      ㉡ 파생어      ㉢ 합성어\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠ 바다 / ㉡ 덮개 / ㉢ 돌다리
\kfChoice ㉠ 덮개 / ㉡ 바다 / ㉢ 돌다리
\kfChoice ㉠ 바다 / ㉡ 돌다리 / ㉢ 덮개
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{밑줄 친 부분이 '접두사'가 \uline{아닌} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      어근 + 어근\end{kfEvidence}
\begin{kfChoices}
\kfChoice 헛수고
\kfChoice 잠꾸러기
\kfChoice 손발
...
\begin{kfEvidence}<보기>      ∙ '돌다리', '책가방', '오가다', '검붉다'는 ( ㉠ )이다.      ∙ '맨손', '풋사과', '지우개', '욕심쟁이'는 ( ㉡ )이다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠ 단일어 / ㉡ 파생어
\kfChoice ㉠ 파생어 / ㉡ 합성어
\kfChoice ㉠ 합성어 / ㉡ 파생어
...
\end{kfQBlock}
\begin{kfQBlock}{9}{}
\kfQStem{밑줄 친 접사의 의미가 바르게 연결되지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{11}{}
\kfQStem{다음 중 '파생어'가 \uline{아닌} 것은?}
\begin{kfChoices}
\kfChoice 덮개
\kfChoice 맨손
\kfChoice 지우개
...
\end{kfQBlock}
\begin{kfQBlock}{12}{}
\kfQStem{다음 중 '합성어'가 \uline{아닌} 것은?}
\begin{kfChoices}
\kfChoice 돌다리
\kfChoice 오가다
\kfChoice 새해
...
\end{kfQBlock}
\begin{kfQBlock}{14}{}
\kfQStem{‘새해’와 ‘큰집’이 합성어인 이유에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>        (가) 덮개, 지우개, 장난꾸러기        (나) 돌다리, 책가방, 오가다\end{kfEvidence}
\begin{kfChoices}
...
\kfStartConcept{개념 영역 — 시간의 흐름에 담긴 의미}


\kfSecHeader{kfAreaConcept}{개념}{시간의 흐름에 담긴 의미}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{이 시의 시상 전개 방식에 대한 설명으로 적절하지 \uline{않은} 것은?}
...
\begin{kfEvidence}<보기>      죽는 날까지 하늘을 우러러      한 점 부끄럼이 없기를,      잎새에 이는 바람에도      나는 괴로워했다.      별을 노래하는 마음으로      모든 죽어 가는 것을 사랑해야지      그리고 오늘 밤에도 별이 바람에 스치운다.      -윤동주 「서시」\end{kfEvidence}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{<보기>의 시상 전개 방식의 효과에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>      어제는 아버지를 잃고, 오늘은 아들을 잃었다. 내일은 또 어떤 슬픔이 나를 기다리고 있을까.\end{kfEvidence}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{1}{}
\kfQStem{이 시에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{이 시의 분위기에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{㉠과 같이 대상을 다른 사물에 빗대어 표현하는 방법의 효과로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{이 시의 표현상 특징으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{<보기>를 참고하여 이 시를 감상한 내용으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>

정지용 시인은 언어를 섬세하게 다듬어 회화적인 이미지를 만들어 내는 데 뛰어난 시인이다. 그는 단어의 소리나 색채, 질감 등을 활용하여 독자가 시를 읽으며 마치 한 폭의 그림을 감상하는 듯한 느낌을 받게 한다.\end{kfEvidence}
\begin{kfChoices}
...
\kfStartNonlit{비문학 영역 — [과학] 방사성 동위원소를 이용한 암석 연대 측정법}


\kfSecHeader{kfAreaNonlit}{지문}{[과학] 방사성 동위원소를 이용한 암석 연대 측정법}
...
\noindent \textbullet\ 한계: 초기의 ( ① )은(는) 정확한 연대를 알 수 없는 상대적 척도였음   \\
\noindent \textbullet\ 발전: 러더포드가 ( ② )을(를) 이용하여 암석의 절대 연대를 계산하는 ( ③ )을(를) 시작함 \\

3문단: 동위원소의 정의 \\

\noindent \textbullet\ 원소 결정: 원자핵 속 ( ④ ) 수에 따라 결정됨   \\
\noindent \textbullet\ ( ⑤ ): 양성자 수는 같지만 ( ⑥ ) 수가 다른 원소 \\

4문단: 방사성 붕괴 \\

\noindent \textbullet\ 정의: 불안정한 원자핵이 방사선을 방출하며 ( ⑦ ) 상태로 변하는 과정   \\
\noindent \textbullet\ 용어: 붕괴 전 원소는 ‘( ⑧ )’, 붕괴 후 원소는 ‘( ⑨ )’ \\

5문단: 반감기 \\

\noindent \textbullet\ 정의: ( ⑩ )의 개수가 원래의 절반으로 줄어드는 데 걸리는 시간   \\
\noindent \textbullet\ 비율 변화 (모원소:자원소):   \\
\noindent \quad\textbullet\ 1회 후: ( ⑪ )   \\
\noindent \quad\textbullet\ 2회 후: ( ⑫ )   \\
\noindent \quad\textbullet\ 3회 후: ( ⑬ ) \\

6문단: 연대 측정의 원리 \\

\noindent \textbullet\ 방법: 암석 속 모원소와 자원소의 ( ⑭ )과(와) 원소의 ( ⑮ )을(를) 이용해 연대 추정   \\
\noindent \textbullet\ 특징: 반감기는 ( ⑯ )이나 ( ⑰ )에 영향을 받지 않음 \\
\end{tcolorbox}\par\medskip
\kfSecHeader{kfAreaNonlit}{문제}{}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{㉠동위원소에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      어떤 화성암 속에 방사성 동위원소 X가 포함되어 있었다. X의 반감기는 4억 년이며, 현재 이 암석 속 X와 이것이 붕괴하여 생성된 Y의 비율은 1 : 7이다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice 4억 년
\kfChoice 8억 년
\kfChoice 12억 년
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{연대 측정 방법에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{윗글의 서술 방식에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\kfStartGrammar{문법 영역 — 단어의 변신, 품사 통용의 이해}


\kfSecHeader{kfAreaGrammar}{개념}{단어의 변신, 품사 통용의 이해}
...
\kfSecHeader{kfAreaGrammar}{활동}{내용 확인}
\noindent\textit{\small 지문의 핵심 내용을 떠올리며 아래 빈칸에 알맞은 말을 채워 보세요. 1. 품사 통용의 개념 | 구분 | 내용 | | :---- | :---- | | 품사 [ ① ] (通用) | ∙ 하나의 단어가 [ ② ] 이상의 품사로 쓰이는 현상. <br>(예: '오늘'이 명사로도, 부사로도 쓰임) | | 중요 구별 기준 | ∙ 문장 안에서 어떤 [ ③ ]을/를 하는지, 특히 [ ④ ]와/과 결합하는지, 다른 말을 꾸미는지 등을 확인해야 함. | 2. 품사 통용의 주요 사례 및 구별 기준 | 사례 | 품사 1 | 기준 1 (예시) | 품사 2 | 기준 2 (예시) | | :---- | :---- | :---- | :---- | :---- | | 수(數) 표현 | [ ⑤ ] | ∙ [ ⑥ ] 와/과 결합함.<br>∙ (예: "사과 다섯이 남았다.") | [ ⑦ ] | ∙ 뒤의 체언(명사) 을 꾸밈.<br>∙ (예: "다섯 개가 남았다.") | | 지시 표현 | [ ⑧ ] | ∙ [ ⑨ ]와/과 결합함.<br>∙ (예: "이는 내가 아낀다.") | [ ⑩ ] | ∙ 뒤의 체언(명사)을 꾸밈.<br>∙ (예: "이 물건은 아낀다.") | | 시간 표현<br> | [ ⑪ ] | ∙ [ ⑫ ]와/과 결합함.<br>∙ (예: "오늘은 잊을 수 없다.") | [ ⑬ ] | ∙ 뒤의 서술어(용언)를 꾸밈.<br>∙ (예: "오늘 만났다.") | | 형태가 같은 용언 | [ ⑭ ] | ∙ 상태를 나타냄.<br>∙ '-(ㄴ)는다' 결합 불가능.<br>∙ (예: "길이 밝다." (X 밝는다)) | [ ⑮ ] | ∙ 동작/변화를 나타냄.<br>∙ '-(ㄴ)는다' 결합 가능.<br>∙ (예: "날이 밝는다." (O)) | | '-만큼', '-뿐', '-대로' | [ ⑯ ] | ∙ 체언 뒤에 붙여 씀.<br>∙ (예: "너만큼 잘한다.") | [ ⑰ ] | ∙ 용언의 관형사형 뒤에 띄어 씀.<br>∙ (예: "아는 만큼 보인다.") |}\par\medskip
...
\begin{multicols}{2}\raggedcolumns\setlength{\columnsep}{12pt}
\begin{kfQBlock}{1}{}
\kfQStem{품사 통용에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      ∙  \uline{다섯}이 왔다.      ∙  \uline{다섯} 사람이 왔다.\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      ㉠ \uline{이것}은 내 책이다.      ㉡ \uline{이} 책은 내 것이다.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠ 대명사 / ㉡ 관형사
\kfChoice ㉠ 관형사 / ㉡ 대명사
\kfChoice ㉠ 수사 / ㉡ 관형사
...
\begin{kfEvidence}<보기>      ㉠ 아이가 쑥쑥 \uline{큰다}.      ㉡ 내 동생은 키가 \uline{크다}.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠: 동사, ㉡: 동사
\kfChoice ㉠: 형용사, ㉡: 형용사
\kfChoice ㉠: 동사, ㉡: 형용사
...
\begin{kfEvidence}<보기>        ㉠ 하늘 / ㉡ 셋 / ㉢ 이 / ㉣ 밝다 / ㉤ 바다\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠, ㉡
\kfChoice ㉠, ㉤
\kfChoice ㉡, ㉢
...
\begin{kfEvidence}<보기>       "\uline{이} 사람은 \uline{셋째} 아들이다."       ㉠ '이'의 품사: ( )       ㉡ '셋째'의 품사: ( )\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠ 대명사 / ㉡ 수사
\kfChoice ㉠ 대명사 / ㉡ 관형사
\kfChoice ㉠ 관형사 / ㉡ 수사
...
\begin{kfEvidence}<보기>       ㉠ 길이 \uline{밝다}.       ㉡ 날이 \uline{밝다}.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠, ㉡ 모두 형용사이다.
\kfChoice ㉠, ㉡ 모두 동사이다.
...
\begin{kfEvidence}<보기>       ∙ ㉠: 조사와 결합하여 '체언'으로 쓰임.       ∙ ㉡: 뒤의 체언을 꾸며 '관형사'로 쓰임.\end{kfEvidence}
\begin{kfChoices}
\kfChoice ㉠:  이는 / ㉡:  이 사람
\kfChoice ㉠:  다섯 사람 / ㉡:  다섯이
\kfChoice ㉠:  오늘 만났다 / ㉡:  오늘의
...
\begin{kfEvidence}(가) 아이 \uline{다섯}이 놀고 있다.   (나) \uline{다섯} 아이가 놀고 있다.\end{kfEvidence}
\kfAnswerNote{답안}{4}
\end{kfQBlock}
\end{multicols}


\kfStartConcept{개념 영역 — 여성 화자를 통한 정서 표현}


\kfSecHeader{kfAreaConcept}{개념}{여성 화자를 통한 정서 표현}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{시인이 여성 화자를 설정하는 이유로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{정철이 「사미인곡」·「속미인곡」에서 여성 화자를 설정한 목적에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      동짓달 기나긴 밤을 한가운데 잘라내어      따뜻한 이불 밑에다 차곡차곡 넣어 뒀다      사랑하는 님 오신 날 밤에 길게길게 펴리라.      * 황진이 「동짓달 기나긴 밤」\end{kfEvidence}
\begin{kfChoices}
\kfChoice 분노
\kfChoice 후회
\kfChoice 절망
...
\end{kfQBlock}
\begin{kfQBlock}{5}{}
\kfQStem{<보기>의 화자에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>      임이여, 물을 건너지 마오.      임은 결국 물을 건너시네.      물에 빠져 죽었으니,      장차 임을 어이할꼬.      * 백수광부의 아내 「공무도하가」\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}<보기>      아아, 님은 갔지마는 나는 님을 보내지      아니하였습니다.      제 곡조를 못 이기는 사랑의 노래는      님의 침묵을 휩싸고 돕니다.      * 한용운 「님의 침묵」\end{kfEvidence}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{1}{}
\kfQStem{이 작품에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{이 시의 화자가 궁극적으로 바라는 것에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{이 작품의 화자에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{6}{}
\kfQStem{화자가 임에게 보내고 싶어 한 소재와 그에 담긴 정서가 바르게 연결되지 \uline{않은} 것은?}
\begin{kfChoices}
\kfChoice 옷 - 사랑과 정성
...
\end{kfQBlock}
\begin{kfQBlock}{7}{}
\kfQStem{화자가 ‘호랑나비’가 되려는 이유에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\begin{kfQBlock}{8}{}
\kfQStem{<보기>를 참고하여 이 시를 감상한 내용으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기> '충신연주지사'는 신하가 임금을 그리워하는 마음을 남녀 간의 사랑에 빗대어 표현한 노래이다. 임금에 대한 충성심을 효과적으로 드러내기 위해, 작가들은 주로 이별한 여성을 화자로 설정하여 임을 향한 변치 않는 사랑을 노래했다.\end{kfEvidence}
\begin{kfChoices}
...
\begin{kfEvidence}<보기> 조선 시대 사대부들은 유교적 이념에 따라 임금에 대한 충성을 매우 중요한 가치로 여겼다. 그들은 임금을 향한 마음을 남녀 간의 사랑에 빗대어 표현하는 경우가 많았는데, 이를 통해 변치 않는 충성심을 효과적으로 드러내고자 했다.\end{kfEvidence}
\begin{kfChoices}
...
\kfStartNonlit{비문학 영역 — [기술] 항원-항체 반응을 이용한 검사용 키트의 원리}


\kfSecHeader{kfAreaNonlit}{지문}{[기술] 항원-항체 반응을 이용한 검사용 키트의 원리}
...
\noindent \textbullet\ 활용: 건강 진단, 범죄 현장 혈흔 조사 등   \\
\noindent \textbullet\ 핵심 원리: ( ① )과 그에 특이적으로 반응하는 ( ② )가 결합하는 면역 반응 (자물쇠와 열쇠 비유) \\

2문단: 측면유동면역분석법(LFIA) 키트의 구조와 이동 \\

\noindent \textbullet\ 구조: 시료 패드 → 결합 패드 → 반응막 → 흡수 패드   \\
\noindent \textbullet\ 결합 패드: ( ③ )을 일으키는 표지 물질이 포함된 복합체가 있음 \\

3문단: 반응막의 구성과 판정 \\

\noindent \textbullet\ ( ④ ): 목표 성분의 유무를 판정하는 선   \\
\noindent \textbullet\ ( ⑤ ): 검사가 제대로 진행되었는지 확인하는 선 \\

4문단: LFIA 키트의 작동 방식 \\

\noindent \textbullet\ 직접 방식: 목표 성분이 있으면 검사선이 ( ⑥ )됨   \\
\noindent \textbullet\ 경쟁 방식: 목표 성분이 충분히 많으면 검사선이 ( ⑦ )되지 않음 \\

5문단: 검사 결과의 종류 \\

\noindent \textbullet\ ( ⑧ ): 키트가 목표 성분이 있다고 판정   \\
\noindent \quad\textbullet\ 진양성: 실제로 목표 성분이 있는 경우 (올바른 경보)   \\
\noindent \quad\textbullet\ ( ⑨ ): 실제로는 목표 성분이 없는 경우 (거짓 경보)   \\
\noindent \textbullet\ ( ⑩ ): 키트가 목표 성분이 없다고 판정했는데 실제로는 있는 경우 (놓친 경보) \\

6문단: 키트의 정확도 측정 지표 \\

\noindent \textbullet\ ( ⑪ ): 목표 성분이 있는 경우, 양성으로 판정한 비율   \\
\noindent \textbullet\ ( ⑫ ): 목표 성분이 없는 경우, 음성으로 판정한 비율 \\
...
\end{kfQBlock}
\begin{kfQBlock}{2}{}
\kfQStem{LFIA 키트의 작동 원리에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{3}{}
\kfQStem{‘민감도’가 특히 중요하게 요구되는 상황에 대한 설명으로 적절하지 \uline{않은} 것은?}
\begin{kfChoices}
...
\end{kfQBlock}
\begin{kfQBlock}{4}{}
\kfQStem{윗글을 바탕으로 <보기>의 코로나19 자가 검사 키트를 이해한 것으로 적절하지 \uline{않은} 것은?}
\begin{kfEvidence}<보기>      코로나19 자가 검사 키트는 검사 결과 창에 C(표준선)와 T(검사선)가 있다. 바이러스가 시료에 포함된 경우, C와 T에 모두 선이 나타나 '양성'으로 판정한다. 바이러스가 없는 경우 C에만 선이 나타난다.\end{kfEvidence}
\begin{kfChoices}
