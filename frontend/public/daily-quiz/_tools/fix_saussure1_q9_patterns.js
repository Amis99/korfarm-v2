// Fix recurring Q9 (표현력) pattern bugs in SAUSSURE_1 daily quizzes.
//
// Conservative: only edits when the exact broken template/answers are detected.
//
// Run:
//   node frontend/public/daily-quiz/_tools/fix_saussure1_q9_patterns.js

const fs = require("fs");
const path = require("path");

const DIR = path.join(__dirname, "..", "saussure1");
...
for (const name of fs.readdirSync(DIR)) {
  ...
  if (
    ...
    if (
      ...
      // Grade 1 wording: keep it very simple.
      lines[1] = "느린 부분에서는 잠깐 쉬었다가 느리게 불렀다.";
      ...
      const newBlank2Id = oldId || `${j.contentId || "dq-s1"}-9-blank-2`;
      const blank2 = {
        id: newBlank2Id,
        choices: [
          { id: "A", text: "고르고" },
